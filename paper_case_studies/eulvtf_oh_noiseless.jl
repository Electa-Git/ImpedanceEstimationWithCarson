...
ie_solver = _PMD.optimizer_with_attributes(Ipopt.Optimizer, "max_cpu_time" => 600., "max_iter" => 8000,  "hsllib"=> HSL_jll.libhsl_path, "linear_solver" => "ma27")
...
timestep_set = find_most_loaded_timesteps(profiles, 50)
for power_mult in [2.]#[1., 2.]
    run_impedance_estimation_eulvtf_noiseless(raw"C:\Users\mvanin\OneDrive - KU Leuven\Desktop\repos\DataDrivenImpedanceEstimationWithCarson\results_ma27/", ie_solver, pf_solver, profiles, timestep_set, length_bounds_percval=0.3, power_mult=power_mult)
end

function run_impedance_estimation_eulvtf_noiseless(result_path::String, ie_solver, pf_solver, profiles::_DF.DataFrame, timestep_set; scenario_id::Int = 1, power_mult::Float64=1., use_length_bounds::Bool=true, length_bounds_percval::Float64=0.10)    
    ...
    loads_with_shunts = [l for (l,load) in data["load"]][1:55]
    buses_with_shunts = [data["load"][l]["load_bus"] for l in loads_with_shunts]
    gs = _RAN.rand(z_pu./[20., 30., 40., 50., 60., 70., 80., 100, 150, 250], 55)
    ...
    _IMP.drop_results(case1, result_path*"/eulvtf_noiseless/", "_power_mult_$(power_mult)_", [], sol, mn_data, timestep_set, scenario_id, add_meas_noise, power_mult, false, false, false, use_length_bounds, length_bounds_percval, imp_est, imp_true, real_volts, real_vas, est_volts, est_vas, mn_data["nw"]["1"]["settings"]["exploit_equal_crossection"], mn_data["nw"]["1"]["settings"]["exploit_squaredness"], mn_data["nw"]["1"]["settings"]["exploit_horizontality"])
    _IMP.drop_shunt_results(case1, result_path*"/eulvtf_noiseless/", "_power_mult_$(power_mult)_", sol, mn_data, scenario_id, shunt_resistive)
